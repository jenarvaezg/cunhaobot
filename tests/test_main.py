...
def test_slack_handler_direct(client):
    with patch("main.handle_slack") as mock_handle_slack:
        ...
        mock_handle_slack.return_value = {"direct": "direct_response", "indirect": "indirect_payload"}
        
        with patch("requests.post") as mock_post:
            # Slack sends payload as form data string
            data = {"payload": json.dumps({"response_url": "http://slack.com/response"})}
            rv = client.post("/slack", data=data)
            
            assert rv.data == b"direct_response"
            mock_post.assert_called_with("http://slack.com/response", json="indirect_payload")
