...
class SlackController(Controller):
    ...
    async def auth_redirect_handler(self, request: Request) -> str:
        ...
        client_id = config.slack_client_id.strip()
        client_secret = config.slack_client_secret.strip()
        redirect_uri = f"{config.base_url}/slack/auth/redirect"
        ...
        logger.info(f"Exchanging Slack code. Redirect URI: {redirect_uri}")
        logger.info(f"Client ID: {client_id[:5]}... (len: {len(client_id)})")
