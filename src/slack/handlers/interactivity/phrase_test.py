...
class TestSlackInteractivity:
    ...
    def test_handle_phrase_shuffle(self):
        ...
        mock_service.get_phrases.return_value = [mock_phrase]

        response = handle_phrase(slack_data, phrase_service=mock_service)
        assert response["direct"]["replace_original"] is True
        assert response["direct"]["attachments"] is not None
        mock_service.get_phrases.assert_called_with(search="search-text", long=True)
