...
class TestPhraseService:
    ...
    @pytest.mark.asyncio
    async def test_generate_sticker(self, service):
        mock_bot = MagicMock()
        p = Phrase(text="test")
        ...
        with (
            ...
            file_id = await service.generate_sticker(p, mock_bot)
            assert file_id == "file_id_123"
            mock_png.assert_called_once()

    @pytest.mark.asyncio
    async def test_generate_sticker_long(self, service):
        mock_bot = MagicMock()
    ...
    async def test_create_from_proposal(self, service):
        ...
        # Mock generate_sticker internal call
        service.generate_sticker = AsyncMock(return_value="sticker_123")
